...
constexpr std::uint32_t kMagic = 0x50595043; // "CPYP"
constexpr std::uint16_t kVersion = 1;

constexpr std::uint16_t kMsgTask = 1;
constexpr std::uint16_t kMsgQuit = 2;
constexpr std::uint16_t kMsgResult = 3;
constexpr std::uint16_t kMsgError = 4;
...
bool WorkerProcess::stop() {
    ...
    if (childInFd_ >= 0) {
        const MessageHeader quit{ kMagic, kVersion, kMsgQuit, 0, 0, 0 };
...
std::vector<double> WorkerProcess::process_chunk(
    ...
    if (response.magic != kMagic || response.version != kVersion) {
