...
TEST_CASE("MessageHeader field offsets are correct", "[protocol]")
{
    ...
    REQUIRE(offsetof(MessageHeader, cols) == 24);
}
...
// Dataset
// ---------------------------------------------------------------------------
TEST_CASE("Dataset default-initialises to zero", "[dataset]")
{
    Dataset ds(4, 3);
...
TEST_CASE("Dataset rows() and cols() are correct", "[dataset]")
{
    Dataset ds(7, 5);
    REQUIRE(ds.rows() == 7);
    REQUIRE(ds.cols() == 5);
...
TEST_CASE("Dataset at() read/write roundtrip", "[dataset]")
{
    Dataset ds(3, 4);
    ds.at(1, 2) = 3.14;
...
TEST_CASE("Dataset at() const overload works", "[dataset]")
{
    Dataset ds(2, 2);
    ds.at(0, 1) = 9.9;
...
TEST_CASE("Dataset row_ptr points to contiguous row data", "[dataset]")
{
    Dataset ds(3, 4);
    ds.at(1, 0) = 1.0;
    ds.at(1, 1) = 2.0;
...
TEST_CASE("Dataset storage is row-major (adjacent rows differ by cols)", "[dataset]")
{
    Dataset ds(3, 5);
...
TEST_CASE("Dataset 1x1 edge case", "[dataset]")
{
    Dataset ds(1, 1);
    REQUIRE(ds.rows() == 1);
    REQUIRE(ds.cols() == 1);
...
TEST_CASE("Dataset row_ptr(0) equals &at(0,0)", "[dataset]")
{
    Dataset ds(2, 3);
    ds.at(0, 0) = 42.0;
